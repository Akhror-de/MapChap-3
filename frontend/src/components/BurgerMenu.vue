...
export default {
  ...
  setup() {
    ...
    const openPanelHandler = (panelName) => {
      console.log('🔄 BurgerMenu: Opening panel:', panelName)
      openPanel(panelName)
    }

    const closeAll = () => {
      console.log('📪 BurgerMenu: Closing all panels')
      closePanel()
      if (isMenuOpen.value) {
        toggleBurgerMenu()
      }
    ...
    const initAuth = () => {
      console.log('🔐 BurgerMenu: Initializing auth')
      initTelegramAuth()
    ...
    const handleLogout = () => {
      if (confirm('Вы уверены, что хотите выйти из аккаунта?')) {
        console.log('🚪 BurgerMenu: Logging out')
        logout()
        closeAll()
...
/* Side Panels - ИСПРАВЛЕННЫЕ СТИЛИ */
.side-panels {
  position: fixed;
  top: 0;
  left: 0;
